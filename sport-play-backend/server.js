...
app.post('/api/auth/vk', async (req, res) => {
  ...
  try {
    const params = { grant_type: 'authorization_code', code, device_id, client_id: '54575533', redirect_uri: 'https://sportplay.458000.ru/auth/vk/callback' }
    if (state) params.state = state
    if (code_verifier) params.code_verifier = code_verifier
    const response = await axios.post('https://id.vk.com/oauth2/auth', new URLSearchParams(params), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
    ...
    const name = (first_name && last_name && first_name !== 'undefined') ? first_name + ' ' + last_name : 'VK пользователь'
    ...
    const token = jwt.sign({ id: user.id, name: user.name }, 'sportplay_secret', { expiresIn: '7d' })
